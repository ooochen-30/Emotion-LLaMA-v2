...
def parse_args():
    ...
    parser.add_argument("--cfg-path", default='eval_configs/temp.yaml',
...
with gr.Blocks(theme=gr.themes.Soft(), css="#chatbot .user {background-color: #EBF5FB;} #chatbot .bot {background-color: #E8F8F5;}") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            chatbot_display = gr.Chatbot(label='Emotion-LLaMA-v2', height=500, elem_id="chatbot")
            
            with gr.Row():
                text_input_box = gr.Textbox(placeholder='Type your question here...', show_label=False, container=False, scale=8,lines=3)
                send_button = gr.Button("▶️ Send", variant='primary', size='sm', scale=1)

    gr.Markdown(introduction)
    
    examples = [
        ["examples/dia_622_utt_8_ch.mp4", "[emotion] Identify the emotion expressed in the video. Choose from: anger, disgust, fear, happy, neutral, sad, surprise."],
        ["examples/cM3Yna7AavY_1.mp4", "[sentiment] Identify the displayed sentiment in the video: Positive, Neutral or Negative."],
        ['examples/video_0019_0002.mp4',"The person in video says: Now, theres two reasons. [reason] Taking into account the speaker's expressions, vocal tone, and spoken words, what sentiment—Neutral, Positive, or Negative—can be inferred from the combined cues?"],
        ["examples/video_0006_0013.mp4","The person in video says: Look who's here. [think] Based on the speaker’s visual and auditory behavior in the video, choose one emotion from: happy, anger, neutral, surprise, fear, sad, disgust. In <think>, describe the visual cues (e.g., eye contact, gestures), the vocal tone (e.g., stressed, calm), and the meaning of what is said. Then, in <answer>, provide your final label."]
